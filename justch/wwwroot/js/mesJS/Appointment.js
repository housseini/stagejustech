...
$().ready(function () {
    ...
    $('#IdMedicalAct').change(function () {
        $.get("/MedicalAct/GetByid", { id: parseInt($("#IdMedicalAct").val()) }, function (re) {
            $.get("/Room/GetByID", { id: re.IdRoom }, function (t) {
                $("#IdRoom1").append('  <option selected  value="' + t.Id + '"> ' + t.Name +'  </option>')
            })
        })
       
    })

    $('#IdMedicalAct111').change(function () {
        $.get("/MedicalAct/GetByid", { id: parseInt($("#IdMedicalAct111").val()) }, function (re) {
            $.get("/Room/GetByID", { id: re.IdRoom }, function (t) {
                $("#IdRoom1111").append('  <option selected  value="' + t.Id + '"> ' + t.Name + '  </option>')
            })
        })
    ...
    $.get('/UTILITY/AllRdv', function (re) {
        ...
        re.forEach(
            function (item) {
                ...
                var end = moment(start).add(1440, 'minutes').format("YYYY-MM-DDTHH:mm:ss");
                ...
                if (item['Appointment']['State'] ==" ANNULER ") {
                    color1="red"
                }
                if (item['Appointment']['State'] == " TERMINER ") {
                    color1 = "green"
                }
                if (item['Appointment']['State'] == " RETARDER ") {
                    color1 = "black"
                }
                if (item['Appointment']['State'] == " EN COURS ") {
                    color1 = "EN COURS"
                }
...
function voirInfo(id) {
    ...
    $.get("/UTILITY/GetRDVbyIdAPP", { id: id }, function (re) {
        $("#Appointementstatue").append('<option selected value="' + re['Appointment'].State + '" >' + re['Appointment'].State+'</option>')
        $("#INFODATE").empty()
        ...
        if (re['Appointment']['State'] ==" EN COURS ") {
            ...
            $("#divbuton").append('<button type="button" class="btn btn-white" onclick="fermer()"><i class="fa fa-  m-r-5"> fermer </i> </button>'+
               '<button type="button" class="btn btn-primary" onclick="edite()" > <i class="fa fa-pencil m-r-5"> Modifier </i> </button > '+
           '<button type = "button" class= "btn  btn-white" onclick = "terminerrendezvous()" > <i class="fa fa-check m-r-5"> terminer </i> </button > '+
        ...
        if (re['Appointment']['State'] == " RETARDER ") {
...
function edite() {
    ...
    class Appointment {
        ...
        constructor() {
            ...
            this.State = $("#Appointementstatue").val()
            if ($("#INFO").val() == '') {
