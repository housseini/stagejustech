...
namespace justch.Models.DAL
{
    public class DAL_ActDataTransfertsEnbryonnaire
    {
        ...
        public static Message Add(List<int> listenumero,int IdMedicalRecordAct,int IdEnbryologisteDoctor,string Date,string heur,string sang,string glaire,string cathere,string transfer,string com,string nombreembroyontr,string Jours,int IdDoctor, int idculture)
        {
            try
            {
                if (DAL_ActDataCongelationEnbryonnaire.GETS(IdMedicalRecordAct).Count != 0)
                {
                    int idcongelation = DAL_ActDataCongelationEnbryonnaire.GETS(IdMedicalRecordAct)[0].Id;
                ...
                string query = "INSERT INTO ActDataTransfertsEnbryonnaire(IdMedicalRecordAct,IdEnbryologisteDoctor,EmbryologisteDoctorType,Date,Heure,Commentaires,NombreEnbryonsTransferes,catheter,Transerfer,Sang,Glaire,IdDoctor)" +
                    "VALUES(@IdMedicalRecordAct, @IdEnbryologisteDoctor, @EmbryologisteDoctorType  , @Date, @Heure,@Commentaires,@NombreEnbryonsTransferes,@catheter,@Transerfer,@Sang,@Glaire,@IdDoctor);";
                ...
                CommandHelper.AddParameterValue<int>(command, "@IdDoctor", IdDoctor);
                CommandHelper.AddParameterValue<string>(command, "@EmbryologisteDoctorType","Embrologite");
                CommandHelper.AddParameterValue<string>(command, "@Date", Date);
                CommandHelper.AddParameterValue<string>(command, "@Heure", heur);
                CommandHelper.AddParameterValue<string>(command, "@catheter", cathere);
                CommandHelper.AddParameterValue<string>(command, "@Sang", sang);
                CommandHelper.AddParameterValue<string>(command, "@Glaire", glaire);
                CommandHelper.AddParameterValue<string>(command, "@Transerfer", transfer);
                CommandHelper.AddParameterValue<string>(command, "@Commentaires", com);
                CommandHelper.AddParameterValue<string>(command, "@NombreEnbryonsTransferes", listenumero.Count.ToString());
                command.ExecuteNonQuery();
                connection.Close();
                var id = GETS(IdMedicalRecordAct)[0].Id;
                if (id != 0)
                {
                    DAL_EnbryonTransfertData.Add(listenumero, Jours, id,glaire);
                }
                DAL_DevenirEmbryon.Update(listenumero, "Transferé", idculture,0,0);
